    ...
    <div className="flex flex-col gap-6 pb-12 h-full max-w-5xl mx-auto w-full pt-4 md:pt-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        ...
        <div className="flex-1 flex flex-col gap-6">
            ...
            {isEditing ? (
              ...
              <div className="flex flex-col gap-5">
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-semibold tracking-widest text-neutral-500 uppercase">Ticket Title</label>
                  <input 
                    type="text"
                    value={editTitle}
                ...
                <div className="flex items-center justify-between border-b border-white/10 pb-6">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <span className="text-sm font-medium text-neutral-500">#{String(ticket.ticket_id).slice(-4)}</span>
          ...
          {/* Conversation Thread */}
          <h3 className="text-sm font-semibold tracking-widest text-neutral-500 uppercase mt-4 mb-2">Conversation</h3>
          
          <GlassCard level={2} className="p-6 flex flex-col gap-6 min-h-[350px]">
            {commentsLoading ? (
              <div className="flex-1 flex items-center justify-center text-neutral-500 text-xs">Loading conversation...</div>
            ) : comments.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center text-neutral-500 p-8 gap-2">
                ...
                <div>
                  <p className="font-semibold text-neutral-400 text-sm">No messages yet</p>
                  <p className="text-xs">Start the conversation by typing in the input composer below.</p>
                </div>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        <div className={`flex items-center gap-2 mb-1 text-[10px] text-neutral-500 ${isOwnComment ? 'justify-end' : ''}`}>
                          <span className="font-medium text-neutral-300">{c.user?.name || 'User'}</span>
                          <span className="px-1.5 py-px text-[8px] bg-white/5 border border-white/10 rounded uppercase font-bold text-neutral-400">{commenterRole}</span>
                          <span>&middot;</span>
                        ...
                        {c.isOptimistic && (
                          <span className="text-[9px] text-neutral-600 italic self-end mt-0.5">Sending...</span>
                        )}
                      </div>
                    </div>
                  );
          ...
          <GlassCard level={2} className="p-6 flex flex-col gap-6">
            ...
            {canManageStatus && validTransitions.length > 0 && (
              <div className="flex flex-col gap-2 border-b border-white/10 pb-6">
                <span className="text-[10px] font-semibold tracking-widest text-neutral-500 uppercase">Context Actions</span>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-white font-medium">{ticket.assigned_to || 'Unassigned'}</span>
                  </div>
                  ...
                  {isAdmin && !ticket.assigned_to && agents.length > 0 && (
                    <div className="mt-2 flex flex-col gap-1.5">
                      <label className="text-[9px] font-semibold tracking-wider text-neutral-500 uppercase">Assign Agent</label>
                      <select 
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <User className="w-4 h-4 text-neutral-400" />
                  <span className="text-sm text-white font-medium">{ticket.created_user?.name || 'Customer'}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-white font-medium">
                    {new Date(ticket.created_at || Date.now()).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
                  </span>
                </div>
              </div>
            </div>
